...
## Prompt 6: Flagged Patients Component (Bonus)

```
Create a React component that fetches and displays only flagged patients from the hospital system.

Requirements:
- Component name: FlaggedPatients
- Fetch from: http://localhost:8001/patients/flagged
- Display data in a plain HTML table with same columns: ID, Full Name, Personal ID, Flag
- Show loading state as plain text "Loading flagged patients..."
- Show error state as plain text "Error loading flagged patients: [error message]"
- If no flagged patients exist, show "No flagged patients found"
- No styling, no CSS classes
- Use the Patient type from src/types.ts
- Handle null values by displaying "N/A"

Generate the complete component in `src/components/FlaggedPatients.tsx`.

Then update App.tsx to include this component under a new <h2> heading "Flagged Patients" after the hospital data section.
```

---

## Prompt 7: Cross-Reference Component (Optional)

```
Create a React component that shows which individuals exist in both systems.

Requirements:
- Component name: CrossReference
- Fetch suspects from http://localhost:8000/suspects
- Fetch patients from http://localhost:8001/patients
- Find matching records by personal_id
- Display matches in a plain HTML table with columns: Personal ID, Name (Police), Name (Hospital), Flag (Police), Flag (Hospital)
- Show loading state while fetching both datasets
- Show error if either fetch fails
- If no matches found, show "No matching records found"
- No styling, no CSS classes

Generate the complete component in `src/components/CrossReference.tsx`.

Then update App.tsx to include this component under a new <h2> heading "Cross-Reference (Matching Records)" at the bottom.
```
